//! End-to-end ingest: take a local replay collection, hash + parse + insert every
//! .kwreplay into a fresh SQLite catalogue, verify it round-trips.
...
fn ingests_corpus_into_sqlite() {
    ...
    let tmpdir = std::env::temp_dir().join(format!("tacitus-ingest-{}.sqlite3", std::process::id()));
    ...
    assert!(report.errors.len() < 20, "too many ingest errors: {}", report.errors.len());
    ...
    println!(
        ...
        report2.scanned, report2.inserted, report2.duplicates, report2.errors.len()
    );
